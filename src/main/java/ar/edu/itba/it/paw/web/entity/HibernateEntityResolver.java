package ar.edu.itba.it.paw.web.entity;
...
import ar.edu.itba.it.paw.model.PersistentEntity;
...
public class HibernateEntityResolver implements EntityResolver {
	...
	public <T> T fetch(Class<T> type, Integer id) {
		try {
			...
			throw new HibernateException("Problem while fetching (" + type.getSimpleName() + ", " + id.toString() + ")",
					ex);
		}
	}
