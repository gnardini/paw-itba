...
public class CommentDatabase extends Database<Comment> {
	...
	public List<Comment> getRestaurantComments(long restaurantId) {
		return doListQuery("select userid, firstname, restaurantid, rating, text from comments, users "
				+ "where restaurantid=" + restaurantId
				+ " and userid=users.id");
	}
	
	public Comment getUserComment(long userId, long restaurantId) {
		return doQuery("select userid, firstname, restaurantid, rating, text from comments, users "
				+ "where userid=" + userId 
